...
# The prepare_text_for_speech function formats the input text according to the
# chosen prompt (summary or optimization) before obtaining the streamed response.
...
# try:
#     with open("prompts/summary_prompt.en.txt", "r", encoding="utf-8") as file:
#         SUMMARY_PROMPT = file.read()
# except Exception as e:
#     print(f"Error reading summary_prompt.txt: {e}")
#     SUMMARY_PROMPT = "You are an audio content specialist creating summaries. Transform the text into flowing natural speech WITHOUT ANY MARKDOWN OR LIST FORMATTING."

# try:
#     with open("prompts/optimization_prompt.en.txt", "r", encoding="utf-8") as file:
#         OPTIMIZATION_PROMPT = file.read()
# except Exception as e:
#     print(f"Error reading optimization_prompt.txt: {e}")
#     OPTIMIZATION_PROMPT = "You are a text-to-speech formatting specialist. Transform the following content into an optimal format for speech synthesis while PRESERVING EXACT CONTENT AND STRUCTURE."
...
#     if provider and create_summary:
#         print("Creating summary")
#         prompt_template = SUMMARY_PROMPT + " " + " ".join(text.split()[:5])
#     elif provider and create_optimization:
#         print("Creating optimization")
#         prompt_template = OPTIMIZATION_PROMPT + " " + " ".join(text.split()[:5])
#     else:
#         # If neither summary nor optimization is requested, return original text.
#         return text
